...
export const Showcase = () => {
  ...
  const [showcaseSelected, setShowcaseSelected] = useState<ShowcaseType[]>(showcase.slice(0, 2))
  function showMore() {
    setIsShownMore(true);
    setShowcaseSelected((prev) => [...prev, ...showcase.slice(2, -1)])
  }
  function showLess() {
    setIsShownMore(false);
    setShowcaseSelected(showcase.slice(0, 2))
  }
    ...
    <section className='flex w-full flex-col gap-5 mb-10'>
      ...
      <div className='flex flex-col gap-0 flex-1 overflow-y-auto'>
        <div className='grid gap-6 grid-cols-1 xl:grid-cols-2 mt-4'>
          {showcaseSelected.map((song, index) => (
              ...
              <div className='flex flex-col'>
                <div className='flex flex-row '>
                  <div className='relative mr-4 h-[70px] w-[70px]'>
                    <Image src={song.imageUrl} alt="song image" width={70} height={70} className='absolute inset-0 rounded-md  border-2 border-orange-1' />
                  ...
                  <div className='mt-2 mr-4 flex flex-col flex-1 max-w-[67%]'>
                    <p className='text-base font-bold line-clamp-1'>
                      <Link href="/song/12" className='cursor-pointer outline-none '>{song.songName}</Link>
                    </p>
